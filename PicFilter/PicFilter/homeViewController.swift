...
class homeViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, ImageSelectionProtocol, UIPopoverPresentationControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate
{
  ...
  let alertController = UIAlertController(title: NSLocalizedString("Select an option",
                                        comment: "Title for string"),
                                        message: nil,
  ...
  override func loadView()
  {
    ...
    self.mainButton.setTitle(NSLocalizedString("Images", comment: "images button"),
                                                        forState: .Normal)
    
    self.mainButton.setTitleColor(UIColor.whiteColor(), forState: .Normal)
    self.mainButton.addTarget(self, action: "mainButtonActivated:",
    ...
    self.metricsForVFL = ["mainImageHeader": 60,
                          "mainImageFooter": 70,
                      "mainImageHorizontal": 5,
                                 "cvHeight":100,
                                 "cvBottom": -120,
                           "mainButtonSize": 25]
    ...
    self.assignConstraintsForViews(rootView, metrics: self.metricsForVFL,
                                               views: views)
  ...
  override func viewDidLoad()
  {
    ...
    if UIImagePickerController.isSourceTypeAvailable(UIImagePickerControllerSourceType.Camera) {
      let cameraOption = UIAlertAction(title: NSLocalizedString("Camera", comment: "camera alert"),
        ...
        self.navigationController?.presentViewController(imagePickerController, animated: true,
                                                                              completion: nil)
      })
    ...
    //  UIAlertActoions
    let galleryAlert = UIAlertAction(title: NSLocalizedString("Gallery", comment: "gallery alert"),
    ...
    let photoAlert = UIAlertAction(title: NSLocalizedString("Photos", comment: "photos alert"),
                                   style: UIAlertActionStyle.Default) { (action) -> Void in
    ...
    let filterAlert = UIAlertAction(title: NSLocalizedString("Filter", comment: "filter alert"),
                                    style: UIAlertActionStyle.Default) { (action) -> Void in
    ...
    // button creation
    self.cancelButton = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.Cancel,
                                                     target: self,
                                                     action: "cancelFilterSelect")
    
    self.doneButton   = UIBarButtonItem(title: "Done",
                                        style: UIBarButtonItemStyle.Done,
                                       target: self,
                                       action: "doneButtonSelected")
    
    self.shareButton  = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.Action,
                                                     target: self,
                                                     action: "shareButtonSelected")
  ...
  func shareButtonSelected()
  {
    ...
    let notSignedInAlert = UIAlertController(title: error,
                                           message: notSignedIn,
    ...
    if(SLComposeViewController.isAvailableForServiceType(SLServiceTypeTwitter))
    {
      ...
      self.presentViewController(composeVC, animated: true,
                                          completion: nil)
    } else {
      let twitterAlert = UIAlertAction(title: okay,
  ...
  func thumbNailSetup()
  {
    ...
    {
      let thumbnail = Thumbnail(filterName: name,
                                imageQueue: self.imageQueue,
  ...
  {
    var size = CGSize(width: 100,
                     height: 100)
   
    UIGraphicsBeginImageContext(size)      //begin drawing
    originalImage.drawInRect(CGRect(x: 0,
                                    y: 0,
                                width: 100,
                               height: 100)) //draw image in the CGRect - draws to context
    ...
    cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell
  {
    ...
    { //only runs if thumbnail is chosen - else wll crash
  ...
  func assignConstraintsForViews(rootView: UIView,
                                  metrics: [String:AnyObject],
                                    views: [String:AnyObject])
  {
    //  mainButton
    let mainButtonPositionX = NSLayoutConstraint.constraintsWithVisualFormat("V:[mainButton(mainButtonSize)]-mainButtonSize-|",
                                                                        options: nil,
                                                                        metrics: metrics,
                                                                          views: views)
    rootView.addConstraints(mainButtonPositionX)
    
    let mainButtonCenter = NSLayoutConstraint(item: mainButton,
                                         attribute: .CenterX,
                                         relatedBy: .Equal,
                                            toItem: rootView,
                                         attribute: .CenterX,
    ...
    let mainImageConstraintVerticle = NSLayoutConstraint.constraintsWithVisualFormat("V:|-mainImageHeader-[mainImage]",
    ...
    let mainIMageConstraintFooter = NSLayoutConstraint.constraintsWithVisualFormat("V:[mainImage]-mainImageFooter-|",
                                                                              options: nil,
                                                                              metrics: metrics,
                                                                                views: views)
    ...
    let collectionViewHeight = NSLayoutConstraint.constraintsWithVisualFormat("V:[collectionView(cvHeight)]",
                                                                         options: nil,
                                                                         metrics: metrics,
                                                                           views: views)
    
    self.collectionView.addConstraints(collectionViewHeight )
    let collectionViewConstraintVerticle = NSLayoutConstraint.constraintsWithVisualFormat("V:[collectionView]-(cvBottom)-|",
                                                                                     options: nil,
                                                                                     metrics: metrics,
                                                                                       views: views)
